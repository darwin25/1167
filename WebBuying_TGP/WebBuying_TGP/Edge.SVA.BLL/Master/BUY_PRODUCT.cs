...
namespace Edge.SVA.BLL
{
	...
	{
		private readonly IBUY_PRODUCT dal=DataAccess.CreateBUY_PRODUCT();
		public BUY_PRODUCT()
		{}
		#region  Method
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Edge.SVA.Model.BUY_PRODUCT GetModelByCache(string ProdCode)
		{
			
			string CacheKey = "BUY_PRODUCTModel-" + ProdCode;
			...
			return (Edge.SVA.Model.BUY_PRODUCT)objModel;
		}
        ...
        {
            return dal.GetListTwoTable(strWhere,orderby,startIndex,endIndex);
        }
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.BUY_PRODUCT> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.BUY_PRODUCT> DataTableToList(DataTable dt)
		{
			List<Edge.SVA.Model.BUY_PRODUCT> modelList = new List<Edge.SVA.Model.BUY_PRODUCT>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Edge.SVA.Model.BUY_PRODUCT model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Edge.SVA.Model.BUY_PRODUCT();
					...
					if(dt.Rows[n]["NewFlag"]!=null && dt.Rows[n]["NewFlag"].ToString()!="")
					{
						model.NewFlag=int.Parse(dt.Rows[n]["NewFlag"].ToString());
					}
					if(dt.Rows[n]["HotSaleFlag"]!=null && dt.Rows[n]["HotSaleFlag"].ToString()!="")
					{
						model.HotSaleFlag=int.Parse(dt.Rows[n]["HotSaleFlag"].ToString());
					}
					modelList.Add(model);
				}
			}
        ...
        {
            return dal.GetRecordTwoTableCount(strWhere);
        }
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
        ...
        {
            return dal.GetStockListByPage(strWhere, orderby, startIndex, endIndex);
        }
         /// <summary>
        /// 求总库存和总金额
        /// 创建人：Lisa
