...
namespace Edge.SVA.BLL
{
	...
	{
		private readonly ISVA_ValueAdjust_D dal=DataAccess.CreateSVA_ValueAdjust_D();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Edge.SVA.Model.SVA_ValueAdjust_D GetModelByCache(string Number,int SeqNo)
		{
			
			string CacheKey = "SVA_ValueAdjust_DModel-" + Number+SeqNo;
			...
			return (Edge.SVA.Model.SVA_ValueAdjust_D)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.SVA_ValueAdjust_D> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.SVA_ValueAdjust_D> DataTableToList(DataTable dt)
		{
			List<Edge.SVA.Model.SVA_ValueAdjust_D> modelList = new List<Edge.SVA.Model.SVA_ValueAdjust_D>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Edge.SVA.Model.SVA_ValueAdjust_D model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Edge.SVA.Model.SVA_ValueAdjust_D();
