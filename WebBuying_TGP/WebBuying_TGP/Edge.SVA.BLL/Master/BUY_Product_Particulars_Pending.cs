...
namespace Edge.SVA.BLL
{
	...
	{
		private readonly IBUY_Product_Particulars_Pending dal=DataAccess.CreateBUY_Product_Particulars_Pending();
        ...
        {
            return dal.GetParticularsPending(ProdCode);
        }
		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Edge.SVA.Model.BUY_Product_Particulars_Pending GetModelByCache(string ProdCode,int LanguageID)
		{
			
			string CacheKey = "BUY_Product_Particulars_PendingModel-" + ProdCode+LanguageID;
			...
			return (Edge.SVA.Model.BUY_Product_Particulars_Pending)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.BUY_Product_Particulars_Pending> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.BUY_Product_Particulars_Pending> DataTableToList(DataTable dt)
		{
			List<Edge.SVA.Model.BUY_Product_Particulars_Pending> modelList = new List<Edge.SVA.Model.BUY_Product_Particulars_Pending>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Edge.SVA.Model.BUY_Product_Particulars_Pending model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Edge.SVA.Model.BUY_Product_Particulars_Pending();
					...
					if(dt.Rows[n]["ProductPriority"]!=null && dt.Rows[n]["ProductPriority"].ToString()!="")
					{
						model.ProductPriority=int.Parse(dt.Rows[n]["ProductPriority"].ToString());
					}
					modelList.Add(model);
				}
			}
