...
namespace Edge.SVA.BLL
{
	...
	/// 店铺类型表
	/// </summary>
	public partial class SVA_Transfer_H
	{
		private readonly ISVA_Transfer_H dal=DataAccess.CreateSVA_Transfer_H();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Edge.SVA.Model.SVA_Transfer_H GetModelByCache(string Number)
		{
			
			string CacheKey = "SVA_Transfer_HModel-" + Number;
			...
			return (Edge.SVA.Model.SVA_Transfer_H)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.SVA_Transfer_H> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.SVA_Transfer_H> DataTableToList(DataTable dt)
		{
			List<Edge.SVA.Model.SVA_Transfer_H> modelList = new List<Edge.SVA.Model.SVA_Transfer_H>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Edge.SVA.Model.SVA_Transfer_H model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Edge.SVA.Model.SVA_Transfer_H();
