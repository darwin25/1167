...
namespace Edge.SVA.BLL
{
	...
	/// PO单主表
	/// </summary>
	public partial class Ord_PickingOrder_H
	{
		private readonly IOrd_PickingOrder_H dal=DataAccess.CreateOrd_PickingOrder_H();
		...
		{
			return dal.Exists(PickingOrderNumber);
		}
		...
		/// 得到一个对象实体
		/// </summary>
		public Edge.SVA.Model.Ord_PickingOrder_H GetModel(string POCode)
		{
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Edge.SVA.Model.Ord_PickingOrder_H GetModelByCache(string POCode)
		{
			
			string CacheKey = "Ord_PickingOrder_HModel-" + POCode;
			...
			return (Edge.SVA.Model.Ord_PickingOrder_H)objModel;
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.Ord_PickingOrder_H> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Edge.SVA.Model.Ord_PickingOrder_H> DataTableToList(DataTable dt)
		{
			List<Edge.SVA.Model.Ord_PickingOrder_H> modelList = new List<Edge.SVA.Model.Ord_PickingOrder_H>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Edge.SVA.Model.Ord_PickingOrder_H model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Edge.SVA.Model.Ord_PickingOrder_H();

                    if (dt.Rows[n]["PickingOrderNumber"] != null && dt.Rows[n]["PickingOrderNumber"].ToString() != "")
                    {
                        model.PickingOrderNumber = dt.Rows[n]["PickingOrderNumber"].ToString();
                    }
