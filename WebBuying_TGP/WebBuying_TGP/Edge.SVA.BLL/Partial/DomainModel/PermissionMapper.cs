...
namespace Edge.SVA.BLL.DomainModel
{
    public class PermissionMapper
    {
        ...
        public void SetWebVirtualPath(strin path)
        {
        ...
        public bool Refresh()
        {
            ...
            try
            {
                ...
                foreach (DataRow item in ds.Tables[0].Rows)
                {
                    ...
                    key = this.virtualPath+FormatPath(item);
        ...
        private string FormatPath(string item)
        {
            string key = item[0].ToString().Trim();
            //key = key.Replace("\\","/");
