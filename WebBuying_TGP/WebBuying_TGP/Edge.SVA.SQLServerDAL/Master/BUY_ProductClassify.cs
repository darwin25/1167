...
{
	/// <summary>
	/// 数据访问类:BUY_ProductClassify
    /// 创建人:Lisa
    /// 创建时间：2016-02-25
	/// </summary>
	public partial class BUY_ProductClassify:IBUY_ProductClassify
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ForeignkeyID", "BUY_ProductClassify"); 
		}
		...
		public bool Exists(string ProdCode,int ForeignkeyID,string ForeignTable)
		{
			...
			strSql.Append("select count(1) from BUY_ProductClassify");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into BUY_ProductClassify(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update BUY_ProductClassify set ");
		...
		public bool Delete(string ProdCode,int ForeignkeyID,string ForeignTable)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from BUY_ProductClassify ");
			strSql.Append(" where ProdCode=@ProdCode and ForeignkeyID=@ForeignkeyID and ForeignTable=@ForeignTable ");
			SqlParameter[] parameters = {
					new SqlParameter("@ProdCode", SqlDbType.VarChar,64),
					new SqlParameter("@ForeignkeyID", SqlDbType.Int,4),
		...
		public Edge.SVA.Model.BUY_ProductClassify GetModel(string ProdCode,int ForeignkeyID,string ForeignTable)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ProdCode,ForeignkeyID,ForeignTable,CreatedOn,CreatedBy,UpdatedOn,UpdatedBy from BUY_ProductClassify ");
			strSql.Append(" where ProdCode=@ProdCode and ForeignkeyID=@ForeignkeyID and ForeignTable=@ForeignTable ");
			SqlParameter[] parameters = {
					new SqlParameter("@ProdCode", SqlDbType.VarChar,64),
					new SqlParameter("@ForeignkeyID", SqlDbType.Int,4),
			...
			Edge.SVA.Model.BUY_ProductClassify model=new Edge.SVA.Model.BUY_ProductClassify();
		...
		/// 得到一个对象实体
		/// </summary>
		public Edge.SVA.Model.BUY_ProductClassify DataRowToModel(DataRow row)
		{
			Edge.SVA.Model.BUY_ProductClassify model=new Edge.SVA.Model.BUY_ProductClassify();
			if (row != null)
			{
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM BUY_ProductClassify ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from BUY_ProductClassify T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "BUY_ProductClassify";
			parameters[1].Value = "ForeignTable";
