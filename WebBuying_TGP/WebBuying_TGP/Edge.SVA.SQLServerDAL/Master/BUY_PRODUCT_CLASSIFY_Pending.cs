...
{
	/// <summary>
	/// 数据访问类:BUY_PRODUCT_CLASSIFY_Pending
    /// 创建人：Lisa
    /// 创建时间：2016-08-08
	/// </summary>
	public partial class BUY_PRODUCT_CLASSIFY_Pending:IBUY_PRODUCT_CLASSIFY_Pending
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ForeignkeyID", "BUY_PRODUCT_CLASSIFY_Pending"); 
		}
		...
		public bool Exists(string ProdCode,int ForeignkeyID,string ForeignTable)
		{
			...
			strSql.Append("select count(1) from BUY_PRODUCT_CLASSIFY_Pending");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into BUY_PRODUCT_CLASSIFY_Pending(");
        ...
        public bool Add(string ProdCode)
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into BUY_PRODUCT_CLASSIFY_Pending ");
            strSql.AppendFormat("select *from BUY_PRODUCT_CLASSIFY where ProdCode='{0}'", ProdCode);
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update BUY_PRODUCT_CLASSIFY_Pending set ");
			strSql.Append("CreatedOn=@CreatedOn,");
			strSql.Append("CreatedBy=@CreatedBy,");
			strSql.Append("UpdatedOn=@UpdatedOn,");
			strSql.Append("UpdatedBy=@UpdatedBy,");
            strSql.Append("ProdCode=@ProdCode,");
            strSql.Append("ForeignkeyID=@ForeignkeyID,");
            strSql.Append("ForeignTable=@ForeignTable");
		...
		public bool Delete(string ProdCode,int ForeignkeyID,string ForeignTable)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from BUY_PRODUCT_CLASSIFY_Pending ");
			strSql.Append(" where ProdCode=@ProdCode and ForeignkeyID=@ForeignkeyID and ForeignTable=@ForeignTable ");
			SqlParameter[] parameters = {
					new SqlParameter("@ProdCode", SqlDbType.VarChar,64),
					new SqlParameter("@ForeignkeyID", SqlDbType.Int,4),
		...
		public Edge.SVA.Model.BUY_PRODUCT_CLASSIFY_Pending GetModel(string ProdCode,int ForeignkeyID,string ForeignTable)
		{
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ProdCode,ForeignkeyID,ForeignTable,CreatedOn,CreatedBy,UpdatedOn,UpdatedBy from BUY_PRODUCT_CLASSIFY_Pending ");
            strSql.Append(" where ProdCode=@ProdCode and ForeignkeyID=@ForeignkeyID and ForeignTable=@ForeignTable ");
            SqlParameter[] parameters = {
					new SqlParameter("@ProdCode", SqlDbType.VarChar,64),
					new SqlParameter("@ForeignkeyID", SqlDbType.Int,4),
        ...
        public Edge.SVA.Model.BUY_PRODUCT_CLASSIFY_Pending GetPRODUCT_CLASSIFY(string ProdCode, string ForeignTable)
        {
            ...
            Edge.SVA.Model.BUY_PRODUCT_CLASSIFY_Pending model = new Edge.SVA.Model.BUY_PRODUCT_CLASSIFY_Pending();
            DataSet ds = DbHelperSQL.Query(strSql.ToString(), parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                return DataRowToModel(ds.Tables[0].Rows[0]);
        ...
        /// 得到一个对象实体
        /// </summary>
        public Edge.SVA.Model.BUY_PRODUCT_CLASSIFY_Pending DataRowToModel(DataRow row)
        {
            Edge.SVA.Model.BUY_PRODUCT_CLASSIFY_Pending model = new Edge.SVA.Model.BUY_PRODUCT_CLASSIFY_Pending();
            if (row != null)
            {
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM BUY_PRODUCT_CLASSIFY_Pending ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from BUY_PRODUCT_CLASSIFY_Pending T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "BUY_PRODUCT_CLASSIFY_Pending";
			parameters[1].Value = "ForeignTable";
