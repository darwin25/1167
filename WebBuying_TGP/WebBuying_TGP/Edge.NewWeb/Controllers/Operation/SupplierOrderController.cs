...
namespace Edge.Web.Controllers.Operation.SupplierOrder
{
    public class SupplierOrderController
    {
        private const string fields = " [POCode],[OrderType],[VendorID],[StoreID],[CurrencyCode],[Note1],[Note2],[Note3],[ExpectDate],[TotalAmount],[VATAmount],[DiscountAmount],[CreatedBusDate],[ApproveBusDate],[ApprovalCode],[ApproveStatus],[ApproveOn],[ApproveBy],[CreatedOn],[CreatedBy],[UpdatedOn],[UpdatedBy]";
        private const string condition = " StoreID IN {0} and (CreatedBy IN {1} or UpdatedBy IN {1} or ApproveBy IN {1})";
        private const string andCondition = " and StoreID IN {0} and (CreatedBy IN {1} or UpdatedBy IN {1} or ApproveBy IN {1})";
        public DataSet GetTransactionList(string strWhere, int pageSize, int pageIndex, out int recodeCount, string sortFieldStr)
        {
            ...
            DataSet ds;
            Edge.SVA.BLL.BUY_PO_H bll = new Edge.SVA.BLL.BUY_PO_H()
            {
                StrWhere = strWhere,
                Order = "CreatedOn",
                Fields = fields,
            ...
            Tools.DataTool.AddBuyingVendorNameByID(ds, "VendorName", "VendorID");
        ...
        public static string Approve(Edge.SVA.Model.BUY_PO_H model, out bool isSuccess)
        {
            isSuccess = false;
            ...
            if (bll.Update(model))
            {
                model = new Edge.SVA.BLL.BUY_PO_H().GetModel(model.POCode);
                isSuccess = true;
        ...
        public static string Void(Edge.SVA.Model.BUY_PO_H model, out bool isSuccess)
        {
            isSuccess = false;
            if (model == null) return "No Data";
