...
namespace Edge.Web.Controllers.Operation.StoreOrderManagement.TransInStoreOrder
{
    public class TransInStoreOrderController
    {
        ...
        public DataSet GetTransactionList(string strWhere, int pageSize, int pageIndex, out int recodeCount, string sortFieldStr)
        {
            ...
            //string storeids = SVASessionInfo.CurrentUser.SqlConditionStoreIDByUser(username);
            if (string.IsNullOrEmpty(strWhere))
            {
                if (string.IsNullOrEmpty(storeids))
                {
                    strWhere = " 1=1 and StoreID = " + storeids;
                }
                else
                {
            ...
            DataSet ds;
            Edge.SVA.BLL.Ord_TransInOrder_H bll = new Edge.SVA.BLL.Ord_TransInOrder_H()            
            {  
                StrWhere = strWhere,
                Order = "CreatedOn",
                Fields = fields,
            ...
            //获取排序字段
            string orderStr = "TransInOrderNumber";
        ...
        public static string Approve(Edge.SVA.Model.Ord_TransInOrder_H model, out bool isSuccess)
        {
            isSuccess = false;
            ...
            if (bll.Update(model))
            {
                model = new Edge.SVA.BLL.Ord_TransInOrder_H().GetModel(model.TransInOrderNumber);
                isSuccess = true;
        ...
        public static string Void(Edge.SVA.Model.Ord_TransInOrder_H model, out bool isSuccess)
        {
            isSuccess = false;
            if (model == null) return "No Data";
