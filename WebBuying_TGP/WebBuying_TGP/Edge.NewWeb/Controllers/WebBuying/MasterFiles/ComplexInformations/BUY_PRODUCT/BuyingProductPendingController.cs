...
{
    public class BuyingProductPendingController
    {
        protected Product_PendingViewModel viewModel = new Product_PendingViewModel();

        public Product_PendingViewModel ViewModel
        {
            get { return viewModel; }
        }
        protected Product_PendingViewModel viewModelPending = new Product_PendingViewModel();
        public void LoadViewModel(string ProdCode)
        {
            Edge.SVA.BLL.BUY_PRODUCT_Pending bll = new Edge.SVA.BLL.BUY_PRODUCT_Pending();
            Edge.SVA.Model.BUY_PRODUCT_Pending model = bll.GetModel(ProdCode);
        ...
        public DataSet GetTransactionList(string strWhere, int pageSize, int pageIndex, out int recodeCount, string sortFieldStr)
        {
            ...
            DataTool.AddPendingStatus(ds, "StatusName", "Status", System.Threading.Thread.CurrentThread.CurrentUICulture.Name.ToLower());
            return ds;
        }
        ...
        public ExecResult Add()
        {
            ...
            try
            {
                Edge.SVA.BLL.BUY_PRODUCT_Pending bll = new SVA.BLL.BUY_PRODUCT_Pending();
        ...
        public static string Approve(Edge.SVA.Model.BUY_PRODUCT_Pending model, out bool isSuccess)
        {
            isSuccess = false;
            if (model == null) return "No Data";

            if (model.Status != 0) return Resources.MessageTips.TheTransactionStatusNotPending;

            model.Status = 1;
            model.ApprovedOn = DateTime.Now;
            model.ApprovedBy = Tools.DALTool.GetCurrentUser().UserID;

            Edge.SVA.BLL.BUY_PRODUCT_Pending bll = new Edge.SVA.BLL.BUY_PRODUCT_Pending();

            if (bll.Update(model))
            {
                model = new Edge.SVA.BLL.BUY_PRODUCT_Pending().GetModel(model.ProdCode);
                isSuccess = true;
                return "Success";
            }
            return "";
        ...
        {
            Edge.SVA.BLL.BUY_PRODUCT_Pending bll = new SVA.BLL.BUY_PRODUCT_Pending();
            List<Edge.SVA.Model.BUY_PRODUCT_Pending> list = bll.GetModelList("ProdCode = '" + strWhere + "'");
            if (list.Count > 0)
            {
